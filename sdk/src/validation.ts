...
export function parseProof(value: unknown): ProofBundle {
  ...
  if (!isHexString(p.txBytes) || p.txBytes.length < 4 || p.txBytes.length % 2 !== 0)
  ...
  return {
    ...
    txBytes: p.txBytes,
    merkleProof: {
      root: bytes32(p.merkleProof.root),
      siblings: p.merkleProof.siblings.map((s) => {
        if (typeof s.isLeft !== "boolean") throw new Error("Invalid Merkle direction.");
        return { hash: bytes32(s.hash), isLeft: s.isLeft };
      }),
    },
    continuityProof: {
      lowerEndpointDigest: bytes32(p.continuityProof.lowerEndpointDigest),
      roots: p.continuityProof.roots.map(bytes32),
    },
  };
}
