...
export class AdvanceClient extends AdvanceReader {
  ...
  createGrant(consumer: string, expiresAt: bigint, notify?: OnTransaction) {
    return this.send(() => this.registry.createGrant(address(consumer), 7, expiresAt), 'GrantCreated', 'grantId', notify);
  }
  revokeGrant(grantId: string, notify?: OnTransaction) {
    return this.send(() => this.registry.revokeGrant(bytes32(grantId)), 'GrantRevoked', 'grantId', notify);
  }
  ...
  submitEvidence(action: number, input: ProofBundle, notify?: OnTransaction) {
    ...
    return this.send(() => this.registry.submitAttestedEvent(action, p.chainKey, p.headerNumber, p.txBytes,
      p.merkleProof.root, p.merkleProof.siblings, p.continuityProof.lowerEndpointDigest, p.continuityProof.roots),
    'EvidenceAccepted', 'evidenceId', notify);
  }
}
