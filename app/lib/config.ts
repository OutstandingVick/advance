...
export interface ConfigInput {registry:string;sourceRegistry:string;lenderA:string;lenderB:string}
export const emptyConfig:ConfigInput={registry:'',sourceRegistry:'',lenderA:'',lenderB:''};
export function deploymentFrom(input:ConfigInput):Deployment {
  ...
  if(lenders[0]===lenders[1])throw new Error('Use two different lender contracts.');
  return {chainId:102031,sourceChainKey:1,registry:address(input.registry),sourceRegistry:address(input.sourceRegistry),lenders};
}
