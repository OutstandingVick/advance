...
const prover = process.env.CREDITCOIN_PROOF_BUILDER_URL ?? 'https://prover.cc3-testnet.creditcoin.network';
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({chainId:Number(chain), supportedChains:chains, proofService:health}, null, 2));
